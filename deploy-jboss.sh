...
func_test_slaves_ip_address(){
    ...
    echo "$PATH_TO_ANSIBLE_SETUP_FILES/$SLAVE_FILE_NAME"
...
func_test_password_file(){
    ...
    echo "$PATH_TO_ANSIBLE_SETUP_FILES/$PASSWORD_FILE_NAME"
    read -p "Enter Selection [y/n]: "
    echo ""
...
#3
# 1=setup master, which will then setup the slaves
...
$PATH_TO_ANSIBLE_SETUP_FILE $ANSIBLE_UN $ANSIBLE_PWD $PROG_USER_SELECTION $PROG_USER_PWD $PATH_TO_SLAVE_FILE $SHOW_EXPECT_SCRIPT_MSG $LOG_FILES_PATH | tee -a /tmp/ansible-setup.log
